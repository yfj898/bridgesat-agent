...
Aggregates outbox health, SQLite episode count, indexed episode count,
...
def _validated_episode(path: Path, student_id: str) -> None:
...
def test_fresh_database_reports_zeroes(db: Path) -> None:
    metrics = memory_consistency_metrics(db)
...
def test_index_outage_affects_success_rate(db: Path) -> None:
    learner = LearnerStore(db)
    student_id, _ = learner.create_student("Ari", 20, 1200)
    _validated_episode(db, student_id)
    ...
    metrics = memory_consistency_metrics(db, index=InMemoryMnemisIndex())
    assert metrics["sqlite_episode_count"] == 1
...
def test_deletion_pending_count_tracks_requests(db: Path) -> None:
    learner = LearnerStore(db)
    student_id, _ = learner.create_student("Ari", 20, 1200)
