...
def register_device(payload: dict) -> DeviceRegistration:
...
def revoke_device(device_id: str, payload: DeviceRevokeRequest) -> dict[str, str]:
    try:
        get_service().revoke_device(device_id, payload.student_id)
...
def sync_events(payload: SyncRequest) -> SyncResponse:
    try:
...
def sync_snapshot(student_id: str) -> SnapshotResponse:
    try:
