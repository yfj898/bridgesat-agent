...
class EventStore:
    ...
    def append_learning_event(
        ...
        *,
        ...
        """Insert an immutable learning event.

        Returns True when inserted, False when the event_id already existed
        (idempotent). Projections must never be re-applied for a duplicate.
        """
...
def run_in_transaction(database_path: Path, fn) -> None:
